...
static struct misc_cdev  *pmisc_cdev;
...
static ssize_t misc_read( struct file *filp, char __user *buf, size_t size, loff_t *offset )
{
	...
	if(p >= misc_size)
		return 0;
	if(count > misc_size - p)
		count = misc_size - p;

/*内核空间->用户空间*/
    if ( copy_to_user(buf,(void *) (dev + p), (unsigned long) count) )
    {
        ret = -EFAULT;
    }
    else
...
static ssize_t misc_write( struct file *filp, const char __user *buf, size_t size, loff_t *offset )
{
	...
	if(copy_from_user((void *)(dev + p),buf,count))
		ret = -EFAULT;
	else
	{
		*offset += count;
...
static int __init misc_init(void)
{
  	...
  	int ret;	
	pmisc_cdev = kmalloc(sizeof(struct misc_cdev),GFP_KERNEL);
	if(!pmisc_cdev){
		ret = -ENOMEM;
	...
	memset(pmisc_cdev,0,sizeof(struct misc_cdev));
